...
| 痛点 | 解决方式 |
|------|---------|
| 看完 2 小时播客，笔记散落各处 | Agent 自动生成结构化 HTML，章节、时间戳、要点、原话引述一网打尽 |
| 想快速定位某个精彩观点 | 交互式目录 + 片段索引网格，点击直接跳转 |
| 外语音频啃不动 | 自动转录 + AI 分析 + 中文字幕烧录，生成中文总结页面 |
| 分享给别人要发一堆截图 | 一个 `.html` 文件即可分享，手机/电脑/离线都能看 |
| 视频平台太多，工具不通用 | YouTube、B站、X 通吃，一个 Skill 全覆盖 |
